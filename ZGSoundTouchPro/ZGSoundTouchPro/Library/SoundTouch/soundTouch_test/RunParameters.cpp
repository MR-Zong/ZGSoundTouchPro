...
// Constructor
RunParameters::RunParameters(char *outFilePath)
{
    
    CFURLRef fileURL = CFBundleCopyResourceURL(CFBundleGetMainBundle(), CFSTR("testZong"), CFSTR("wav"), NULL);
    CFStringRef filePath = CFURLCopyFileSystemPath(fileURL, kCFURLPOSIXPathStyle);
    CFStringEncoding encodingMethod = CFStringGetSystemEncoding();
    const char *path = CFStringGetCStringPtr(filePath, encodingMethod);

    string inFileNameStr = string(path);
    unsigned long len = inFileNameStr.length();
    inFileName = (char *)malloc((len+1)*sizeof(char));
    inFileNameStr.copy(inFileName,len,0);
    
    
    outFileName = outFilePath;

//    string outFileNameStr = "result.wav";
//    unsigned long len2 = outFileNameStr.length();
